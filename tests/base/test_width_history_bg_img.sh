...
main() {
  ...
  #
  #
  local -r IMGPNG_ROOT="/Users/likhobabin_im/ws/tmp/imgresizer/desktop/16_9/steilgut-history-pg/2020"
...
#  local RESOLUTION_MAP=('hd:1280-720' 'lg:1024-576' 'md:768-432' 'sm:640-360')
local RESOLUTION_MAP=('xs:320-160')
...
for key in "${!RESOLUTION_MAP[@]}"; do
  ...
  if [ ${is_res_dir_exist} -eq 0 ]; then
    ...
    gmk convert -resize "${res_width}x" +profile "*" -quality ${IMG_JPG_QUALITY} ${IMGPNG_ORIG} "${res_dir}/${res_name}-${BASENAME}.jpg"
    gmk convert -resize "${res_width_2x}x" +profile "*" -quality ${IMG_JPG_QUALITY} ${IMGPNG_ORIG} "${res_dir}/${res_name}-2x-${BASENAME}.jpg"
  else
