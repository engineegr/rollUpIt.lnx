...
main() {
  ...
  #
  #
  local -r IMGPNG_ROOT=/Users/likhobabin_im/ws/tmp/imgresizer/desktop/16_9/news-body-pg
  local -r IMGPNG_ORIG=${IMGPNG_ROOT}/16_9-fhd-2x-bg-news-body-pg.png
  local -r IMG_JPG_QUALITY=60

  local -r BASENAME='bg-news-body-pg'
  local -r ASPECT_RATIO='16_9'
  local val=''
  local res_name=''
  local res_val=''
  local res_width=''
  ...
  local RESOLUTION_MAP=('fhd:1920-1080' 'hdplus:1600-900' 'wxga:1366-768' 'hd:1280-720' 'lg:1024-576' 'md:768-432' 'sm:640-360')
  ...
  for key in "${!RESOLUTION_MAP[@]}"; do
    ...
    if [ ${is_res_dir_exist} -eq 0 ]; then
      ...
      # gmk convert -resize "${res_width}x${res_height}" +profile "*" ${IMGPNG_ORIG} "${res_dir}/png/16_9-${res_name}-${BASENAME}.png"
      # gmk convert -resize "${res_width_2x}x${res_height_2x}" +profile "*" ${IMGPNG_ORIG} "${res_dir}/png/16_9-${res_name}-2x-${BASENAME}.png"
      gmk convert -resize "${res_width}x${res_height}" +profile "*" -quality ${IMG_JPG_QUALITY} ${IMGPNG_ORIG} "${res_dir}/jpg/16_9-${res_name}-${BASENAME}.jpg"
      gmk convert -resize "${res_width_2x}x${res_height_2x}" +profile "*" -quality ${IMG_JPG_QUALITY} ${IMGPNG_ORIG} "${res_dir}/jpg/16_9-${res_name}-2x-${BASENAME}.jpg"
    else
