...
main() {
  ...
  #
  local -r IMGPNG_ROOT=~/ws/tmp/imgresizer/mobile/landscape/news-body-pg
  local -r IMGPNG_ORIG=${IMGPNG_ROOT}/16_9-news-body-pg.jpg
  ...
  local RESOLUTION_MAP=('wxga:1366-768' 'hd:1280-720' 'lg:1024-576' 'md:768-432' 'sm:640-360')
  ...
  for key in "${!RESOLUTION_MAP[@]}"; do
    ...
    if [ ${is_res_dir_exist} -eq 0 ]; then
      ...
      gmk convert -resize "${res_width}x${res_height}" +profile "*" ${IMGPNG_ORIG} "${res_dir}/png/${ASPECT_RATIO}-${res_name}-${BASENAME}.png"
      gmk convert -resize "${res_width_2x}x${res_height_2x}" +profile "*" ${IMGPNG_ORIG} "${res_dir}/png/${ASPECT_RATIO}-${res_name}-2x-${BASENAME}.png"
      gmk convert -resize "${res_width}x${res_height}" +profile "*" -quality ${IMG_JPG_QUALITY} ${IMGPNG_ORIG} "${res_dir}/jpg/${ASPECT_RATIO}-${res_name}-${BASENAME}.jpg"
      gmk convert -resize "${res_width_2x}x${res_height_2x}" +profile "*" -quality ${IMG_JPG_QUALITY} ${IMGPNG_ORIG} "${res_dir}/jpg/${ASPECT_RATIO}-${res_name}-2x-${BASENAME}.jpg"
    else
