...
main() {
  ...
  #
  #
  local -r IMGPNG_ROOT=/Users/likhobabin_im/ws/tmp/imgresizer/desktop/4_3/steilgut-history
  local -r IMGPNG_ORIG=${IMGPNG_ROOT}/bg-history.jpg
  local -r IMG_JPG_QUALITY=60

  local -r BASENAME='bg-history'
  ...
  local RESOLUTION_MAP=('xl:1440-1024' 'hd:1280-720' 'lg:1024-768' 'md:768-546' 'sm:640-455' 'xs:320-640')
  ...
  for key in "${!RESOLUTION_MAP[@]}"; do
    ...
    if [ ${is_res_dir_exist} -eq 0 ]; then
      ...
      # for history photos
      local res_dir=${IMGPNG_ROOT}/"${res_name}"
      ...
      # gmk convert -resize "${res_width}x${res_height}" +profile "*" -quality ${IMG_JPG_QUALITY} ${IMGPNG_ORIG} "${res_dir}/jpg/4_3-${res_name}-${BASENAME}.jpg"
      # gmk convert -resize "${res_width_2x}x${res_height_2x}" +profile "*" -quality ${IMG_JPG_QUALITY} ${IMGPNG_ORIG} "${res_dir}/jpg/4_3-${res_name}-2x-${BASENAME}.jpg"

      # for history photos
      gmk convert -resize "${res_width}x${res_height}" +profile "*" -quality ${IMG_JPG_QUALITY} ${IMGPNG_ORIG} "${res_dir}/${res_name}-${BASENAME}.jpg"
      gmk convert -resize "${res_width_2x}x${res_height_2x}" +profile "*" -quality ${IMG_JPG_QUALITY} ${IMGPNG_ORIG} "${res_dir}/${res_name}-2x-${BASENAME}.jpg"
    else
