...
main() {
  ...
  local -r IMGPNG_ORIG=${IMGPNG_ROOT}/16_9-fhd-2x-bg-left-news-body-pg.jpg
  ...
  for key in "${!RESOLUTION_MAP[@]}"; do
    ...
    if [ ${is_res_dir_exist} -eq 0 ]; then
      ...
      gmk convert -resize "x${res_height}" +profile "*" -quality ${IMG_JPG_QUALITY} ${IMGPNG_ORIG} "${res_dir}/jpg/16_9-${res_name}-${BASENAME}.jpg"
      gmk convert -resize "x${res_height_2x}" +profile "*" -quality ${IMG_JPG_QUALITY} ${IMGPNG_ORIG} "${res_dir}/jpg/16_9-${res_name}-2x-${BASENAME}.jpg"
    else
