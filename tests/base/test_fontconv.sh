...
main() {
  ...
  for ttf_font_fp in $(find ${FONTCONV_ROOT} -type f -regex ".*ttf"); do
    ...
    woff2_font_fp="$(echo ${ttf_font_fp} | sed -E 's/(.*).ttf/\1/g').woff2"
    file_dir=${ttf_font_fp%/*}
    cd ${file_dir}
    echo "$debug_prefix WOFF2 Filepath: ${woff2_font_fp}"
    echo "$debug_prefix WOFF2 Filename dir: ${file_dir}"

    /usr/local/bin/woff2_compress "${ttf_font_fp}" 
  done
